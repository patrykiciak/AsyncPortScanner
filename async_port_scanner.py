...
from root.tools.port_parser import PortParser


class TcpPortScanner:

    def __init__(self, connection_timeout=2, queue_size=0):
        self.connection_timeout = connection_timeout
        self.queue_size = queue_size

    @staticmethod
    def print_numbers(scan_result):
        scanned_number, open_number, filtered_number, closed_number = TcpPortScanner.get_numbers(scan_result)
        print('Number of scanned ports:', scanned_number)
        print('Number of open ports:', open_number)
        print('Number of filtered ports:', filtered_number)
        print('Number of closed ports: ', closed_number)
    ...
    def get_numbers(scan_result):
        ...
        for host in scan_result:
            for port, state, service in scan_result[host]:
                ...
                if state == "closed":
                    scanned += 1
                    closed += 1
        return scanned, opened, filtered, closed

    @staticmethod
    def __create_queue(input_targets):
        scan_result = dict()
    ...
    async def __do_port_tasks(self, port_tasks, scan_result):
        while port_tasks.qsize():
            task = await port_tasks.get()
            ip, port, state, service = await task
            if state == "error":  # and type(service) is OSError and service.args[0] == 10055:
                port_tasks.put_nowait(asyncio.create_task(self.check_port(ip, port)))
                return False
            scan_result[ip].append((port, state, service))
    ...
    async def check_port(self, ip, port):
        ...
        try:
            ...
            try:
                ...
                print(ip, port, 'open', service)
                return ip, port, 'open', service
            except OSError:
                print(ip, port, 'open', 'unknown')
                return ip, port, 'open', 'unknown'
        except (PermissionError, ConnectionRefusedError) as e:
            return ip, port, 'closed', e
        except OSError as e:
            if e.args[0] == 22:
                return ip, port, 'filtered', e
            return ip, port, 'error', e
    ...
    def start_scan(self, input_targets: dict):
        targets, scan_result = self.__create_queue(input_targets)
        loop = asyncio.get_event_loop()
        scan_result = loop.run_until_complete(self.__scan_many(targets, scan_result))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        target_ports = ("1,3-4,6-7,9,13,17,19-26,30,32-33,37,42-43,49,53,70,79-85,88-90,99-100,106,109-111,113,119,"
        ...
        target_ports = PortParser.parse_ports(target_ports)
    else:
        target_ports = PortParser.parse_ports(sys.argv[2])

    start_time = time.time()
    result = scanner.start_scan({domain_address: target_ports})
    print('\nTime taken:', time.time() - start_time)

    TcpPortScanner.print_numbers(result)
